...
class _AddPaymentState extends State<AddPayment> {
  ...
  bool _validateInputs() {
    ...
    if (cardNumber.length != 16 || !RegExp(r'^[0-9]+$').hasMatch(cardNumber)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context).translate('invalid_card_number'))),
      );
      return false;
    }
    ...
    if (!RegExp(r'^(0[1-9]|1[0-2])\/\d{2}$').hasMatch(expiry)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context).translate('invalid_expiry_date'))),
      );
      return false;
    }
    ...
    if (cvv.length != 3 || !RegExp(r'^[0-9]+$').hasMatch(cvv)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context).translate('invalid_cvv'))),
      );
      return false;
    }
  ...
  Future<void> _saveCardToFirestore() async {
    ...
    if (user == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context).translate('login_to_save_card'))),
      );
      return;
    }
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(AppLocalizations.of(context).translate('card_saved_successfully')),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${AppLocalizations.of(context).translate('error_saving_card')}: $e'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          AppLocalizations.of(context).translate('add_payment_card'),
          ...
          SingleChildScrollView(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    AppLocalizations.of(context).translate('add_card_details'),
                    style: const TextStyle(
                      fontSize: 16,
                  ...
                  _buildTextField(
                    AppLocalizations.of(context).translate('name_on_card'),
